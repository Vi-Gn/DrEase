...
class FramedTable(TFrame):
  ...
  def InitTableData(self, absPath: str = '', show = 'headings', selectmode = "extended"):
    ...
    if len(self.columns) >= 1:      
      ...
      self.table.bind("<Escape>", lambda e : self.LoadDataTable())
      self.table.bind("<F3>", lambda e : self.SearchByLabel())
  ...
  def InitTableFile(self, absPath: str, show = 'headings', title: str = "File", selectmode = "browse") -> TTreeview:
...
########################################################################################
  ...
  # remove selected item from file structure table
  def RemoveSelectedItems(self, *args):
    if self.FileOrData:
      CLog.Warn(f"This is a beta function should not be used in production | File : {__file__}, Function : {self.RemoveSelectedItems.__name__}")
      ...
      path = ' '.join(selectedItems["values"])
      ...
      if confirm:
        ...
        if path != '':
          ...
          CLog.Error(f"Can't Remove Main Directory!")
        return
      return
  ...
  def OpenDirectoryFileTable(self, absPath: str = '-1'):
    ...
    if absPath == '-1':
      # 
      absPath = filedialog.askdirectory(initialdir=self.fileManager.GetAbsolutePath(), mustexist=True)
      if absPath != '':
        self.fileManager.SetPathWorkdir(absPath)
      else:
